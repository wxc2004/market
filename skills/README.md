...
## Current Skills (21 test skills for pagination testing)
...
- `test-skill-3` ~ `test-skill-20` — 18 pagination test skills (generated by `scripts/generate-test-skills.cjs`)

### Batch Publish
```bash
# Publish all skills at once via GitHub Actions:
# Go to Actions → "Publish All Test Skills" → Run workflow
```