...
在顶栏新增主题选择器，支持 **8 套配色方案**，即时切换并持久化保存。
...
- 长时间使用 GUI 时切换至暖棕/墨青/柔灰等护眼主题可减轻眼部疲劳
- 选择偏好颜色适配个人审美
- 主题选择存储在 `localStorage`，关闭页面后自动恢复

### 改动文件

| 文件 | 变更 |
|------|------|
| `gui/style.css` | 移除单一 `:root` 变量块；新增 8 套 `body.theme-*` 主题变量 |
| `gui/index.html` | 顶栏新增 `<select id="theme-select">` 主题下拉框 |
| `gui/app.js` | 新增 `THEMES` 数组、`detectTheme()`、`setTheme()`、`applyTheme()`；DOMContentLoaded 中初始化主题 |
...
## ✨ 优化：GUI 上传选择 zip 文件后增加视觉反馈

**问题**: 用户在 Upload 页面选择 zip 压缩包后，没有任何提示表明文件已选中，只能看到"上传并解析"按钮从不灰变为可点，体验模糊。

**修复**: 选中文件后拖拽区显示绿色边框 + 绿色徽章，显示文件名和大小：

| 状态 | 效果 |
|------|------|
| 未选择 | 默认虚线边框 + "放入 zip 或点击选择" |
| 已选择 | 绿色边框 + 绿色背景 + `已选择: xxx.zip (1.2 MB)` |
| 拖拽悬停 | 红色高亮（drag-over，已存在） |
| 重置/丢弃 | 恢复默认状态 |

触发方式覆盖：点击文件选择、拖拽放下、提交时重选都会更新反馈。中英文均适配。

### 改动文件

| 文件 | 变更 |
|------|------|
| `gui/index.html` | 拖拽区内新增 `#upload-file-info` 显示元素 |
| `gui/style.css` | 新增 `.upload-dropzone.selected` 边框/背景样式；`.upload-file-info` 绿色徽章样式 |
| `gui/app.js` | 新增 `formatFileSize()`、`showUploadFileInfo()`、`clearUploadFileInfo()`；3 处入口接入 |
| `CHANGELOG.md` | v1.3.43 更新日志 |
| `package.json` | 版本升至 1.3.43 |
